...
- Video live wallpapers keep an unconfirmed candidate separate from the active asset. The candidate is promoted only when the plugin opens the live-wallpaper preview, so cancelling the system preview (or restarting the wallpaper engine) no longer plays a video the user never accepted.
